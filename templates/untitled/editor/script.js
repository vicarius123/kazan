...
jQuery(function ($) {
    ...
    function updateHref(uri, key, value) {
    ...
    if (topHref.indexOf('editor=1') !== -1 && topHref.indexOf('_templates') !== -1) {
        if (siteHref.indexOf('is_preview') == -1) {
            siteHref = updateHref(siteHref, 'is_preview', 'on');
            themeParts = topHref.match(/theme=([^&]+)/);
            if (themeParts)
