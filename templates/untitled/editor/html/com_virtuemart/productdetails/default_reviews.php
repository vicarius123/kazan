...
<div class="data-control-id-2519 bd-productreviews-1">
    <?php if ($this->showReview):	?>
        <div class="data-control-id-2461 bd-container-39 bd-tagstyles">
            ...
            if ($this->rating_reviews) {
                foreach ($this->rating_reviews as $review) {
                    ...
                    if (!empty($this->rating_reviews) && $review->published) {
                        $reviews_published++;
                        ?>
                        <div class="data-control-id-2463 bd-productreview-1">
    <h4 class="data-control-id-2378 bd-reviewheader-1">
    <?php echo $review->customer; ?>
</h4>
	
		<div class="data-control-id-2367 bd-reviewmetadata-1">
    <?php echo JHTML::date ($review->created_on, JText::_ ('DATE_FORMAT_LC')); ?>
</div>
	
		<div class="data-control-id-2368 bd-reviewtext-1">
    <?php echo $review->comment ?>
</div>
	
		<div class='data-control-id-2376 bd-rating'>
    <?php for ($i = 1; $i <= $maxrating; $i++) : ?>
    <?php $active = ($i <= $review->review_rating ? ' active' : ''); ?>
    <span class='data-control-id-2375  bd-icon bd-icon-3 <?php echo $active ?>'></span>
    <?php endfor; ?>
</div>
</div>
                        <?php
    ...
    <?php endif; ?>
    <div class="data-control-id-2518 bd-reviewform-1">
    ...
    <form method="post" action="<?php echo JRoute::_ ('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=' . $this->product->virtuemart_product_id . '&virtuemart_category_id=' . $this->product->virtuemart_category_id); ?>" name="reviewForm" id="reviewform">
            ...
            ?>
            <div class="data-control-id-2495 bd-container-40 bd-tagstyles">
                <h4>
            ...
            <br/>
            <textarea class="data-control-id-2496 bd-bootstrapinput form-control" title="<?php echo JText::_ ('COM_VIRTUEMART_WRITE_REVIEW') ?>"  id="comment" onblur="refresh_counter();" onfocus="refresh_counter();" onkeyup="refresh_counter();" name="comment" rows="5" cols="60"><?php if (!empty($this->review->comment)) {
            ...
            <div class="review-rating">
                ...
                <br/>
                <div class='data-control-id-2508 bd-rating'>
    <?php for ($i = 1; $i <= $maxrating; $i++) : ?>
    <span class="data-control-id-2507  bd-icon bd-icon-3 active"></span>
    <?php endfor; ?>
</div>
            ...
            <div class="review-count">
                <span><?php echo JText::_ ('COM_VIRTUEMART_REVIEW_COUNT') ?>
                    <input type="text" value="0" size="4" class="data-control-id-2496 bd-bootstrapinput form-control" name="counter" maxlength="4" readonly="readonly"/>
                </span>
            </div>
            <br style="clear:both"/><br/>
            <div class="review-button">
                <input class="data-control-id-2500 bd-button" type="submit" onclick="return( check_reviewform());" name="submit_review" title="<?php echo JText::_ ('COM_VIRTUEMART_REVIEW_SUBMIT')  ?>" value="<?php echo JText::_ ('COM_VIRTUEMART_REVIEW_SUBMIT')  ?>"/>
            </div>
            <?php
            } else {
