...
class Designer_Plugins_Installer
{
    public function execute($params)
    {
        ...
        if (version_compare(JVERSION, '3.0', '<')) {
            ...
            $ret = JArchive::extract($pluginsFolder . $pluginName . '.zip', $pluginsFolder);
            if ($ret === false) {
            ...
            try {
                JArchive::extract($pluginsFolder . $pluginName . '.zip', $pluginsFolder);
            } catch (Exception $e) {
                echo $e->getMessage();
                return;
            }
        ...
        $pluginFolder = $pluginsFolder . $pluginName;
        if (version_compare(JVERSION, '3.0', '<')) {
        ...
        $query->where('element = ' . $db->quote('themler' . $pluginName));
        $db->setQuery($query);
        $db->query();

        jimport('joomla.filesystem.folder');
        JFolder::delete($pluginsFolder . '/' . $pluginName);
